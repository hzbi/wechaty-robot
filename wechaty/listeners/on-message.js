import { getContactTextReply, getRoomTextReply } from '../utils/message'
 ...
 * @param {*} msg 消息主体
 */
const dispatchFriendFilterByMsgType = async (that, msg) => {
    try {
        const type = msg.type()
        ...
        switch (type) {
            case that.Message.Type.Text:
                ...
                if (!isOfficial) {
                    ...
                    if (content.trim()) {
                        reply = await getContactTextReply(that, contact, content)
 ...
 * @param {*} msg 消息主体
 */
const dispatchRoomFilterByMsgType = async (that, room, msg) => {
    const contact = msg.talker() // 发消息人
    ...
    switch (type) {
        case that.Message.Type.Text:
            ...
            if (mentionSelf) {
                ...
                reply = await getRoomTextReply(content, contactName, contact.id, contactAvatar)
                console.log('回复内容', reply)
                if (reply !== '') {
                    await delay(1000)
